...
class TestState(unittest.TestCase):
    ...
    def test_get_status(self):
        with app.test_client() as ress:
            res = ress.get('/api/v1/states/random_id/cities')
            self.assertEqual(res.status_code, 404)
            a_state = [state.id for state in storage.all(State).values()]
            res = ress.get('/api/v1/states/{}/cities'.format(a_state[0]))
            self.assertEqual(200, res.status_code)

    def test_get_response_data(self):
        city = [city.id for city in storage.all(City).values()]
        with app.test_client().get('/api/v1/cities/random_id') as res:
            self.assertEqual(404, res.status_code)
        with app.test_client().get('/api/v1/cities/{}'.format(city[0])) as res:
            self.assertEqual(200, res.status_code)
